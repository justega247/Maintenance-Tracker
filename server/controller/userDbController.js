...
class Users {
  ...
  static addUser(req, res) {
    ...
    pool.query(checkDetails).then(detailsFound => {
      if (detailsFound.rowCount !== 0) {
        return sendMessage(
          res,
          409,
          'fail',
          'Sorry, the username and/or email you supplied, already exists'
        );
      }
      return pool.query(newUser).then(createdUser => {
        const token = sign(
          {
  ...
  static signinUser(req, res) {
    ...
    const token = sign({ id }, SECRET, { expiresIn: TOKEN_EXPIRATION_TIME });
    res
      .header('authorization', token)
      .status(200)
      .json({
  ...
  static addRequest(req, res) {
    ...
    pool.query(getUserRequests).then(userRequests => {
      if (userRequests.rowCount !== 0) {
        const request = userRequests.rows.find(
          singleRequest =>
            singleRequest.title === title &&
            singleRequest.description === description &&
            singleRequest.type === type
        );
        if (request) {
          sendMessage(
            res,
            400,
            'fail',
            'Sorry, you already have a request with those details'
          );
          return null;
        }
      }
      pool
        ...
        .then(newRequest =>
          sendSuccess(
            res,
            201,
            'A new request was just created',
            newRequest.rows[0]
          )
        );
      return null;
    ...
    pool.query(getUserRequests).then(userRequests => {
      if (userRequests.rowCount === 0) {
        sendMessage(
          ...
          200,
          'success',
          'Sorry,you have not made any requests'
        );
        return null;
      }
      sendSuccess(
        res,
        200,
        'Your requests have been retrieved',
        userRequests.rows
      );
      return null;
    });
  ...
  static updateRequest(req, res) {
    ...
    const emptyString = updateValueArray.find(
      updateValue => req.body[updateValue].trim() === ''
    );
    ...
    if (emptyString) {
      sendMessage(
        ...
        400,
        'fail',
        'Sorry, one or more of your update value is empty'
      );
      return null;
    }
    ...
    pool.query(getUserRequests).then(userRequests => {
      ...
      return pool.query(requestToUpdate).then(requestFound => {
        if (requestFound.rowCount === 0) {
          sendMessage(
            res,
            404,
            'fail',
            'Sorry, you do not have a request with that id'
          );
          return null;
        }
        ...
        const updateTheRequest = `UPDATE requests SET title = '${
          updatedRequest.title
        }', type = '${updatedRequest.type}', description = '${
          updatedRequest.description
        }' WHERE requests.id = '${requestId}' RETURNING *`;
        pool
          .query(updateTheRequest)
          .then(updatedFoundRequest =>
            sendSuccess(
              res,
              200,
              'Your request has been updated',
              updatedFoundRequest.rows[0]
            )
          );
        return null;
      });
